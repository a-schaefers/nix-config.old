...
postinstall_overlay_file="postinstall.sh"      # run arbritrary code after nixos-install and before umount /mnt.
...
nix_install_opts="--max-jobs 4 --cores 0 --no-root-passwd"
